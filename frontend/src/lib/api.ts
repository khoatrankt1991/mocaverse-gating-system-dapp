...
export async function verifyInviteCode(
  ...
  const response = await fetch(
    `${API_URL}/api/verify-code?code=${encodeURIComponent(code)}`
  )
...
export async function checkNFTEligibility(
  wallet: string
): Promise<NFTEligibilityResponse> {
  const response = await fetch(
    `${API_URL}/api/checkWallet?wallet=${encodeURIComponent(wallet)}`
  )
